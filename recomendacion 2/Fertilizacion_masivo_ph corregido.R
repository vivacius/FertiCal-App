...
semana_r <- "Semana 9"#-------------------------------------------------------------------------------------------------------------------------IMPORTANTE CAMBIAR SEMANA

# -------------------------------------------------------------------------
# Configuración de abonadoras---------------------------------------------
# -------------------------------------------------------------------------
# Capacidades de tolvas para cada tipo de abonadora (en kg)
capacidades_tolvas <- list(
  VERION = c(500, 300, 300),  # Tolva 1, 2, 3
  TT = c(550, 300)             # Tolva 1, 2
)
...
RCMP<-function(tablones, hacienda, suerte, variedad, compost, vinaza, TCHE, porce_comp, porc_vinaza){
  ...
  if(inventario_muestreo$`%`[inventario_muestreo$Cod==hacienda]==1){
    ...
    #pts_mstr<-readOGR("C:/Users/sacorreac/OneDrive - Sector Agro/AP/6. FERTILIZACION TV/Datos_muestreo_optimizado/Ptos_actual.shp")
    pts_mstr <- remove.duplicates(pts_mstr, zero = 1, remove.second = TRUE)
  ...
  if(length(which(tablones@data$Hac==hacienda & tablones@data$Ste==suerte))==0 || length(capa_suerte[which(capa_suerte$Hac==hacienda & capa_suerte$Ste==suerte),]) == 0){
    ...
    if(length(CM$Dif_Area[which(CM$HACIENDA==hacienda & CM$SUERTE==suerte)])==0){
      ...
      area_ref <- CM$AREA[which(CM$HACIENDA==hacienda & CM$SUERTE==suerte)][pos]
      dif_area <- CM$Dif_Area[which(CM$HACIENDA==hacienda & CM$SUERTE==suerte)][pos]
      
      ver <- abs(dif_area) / area_ref
      
    }
    
    #y <- as.character(year(Sys.Date()))
    y <- "2025"
    #----------------------------SIMMPRO MAPAS DE PRODUCTIVIDAD----------------------------
    if (file.exists(paste0("C:/Users/sacorreac/OneDrive - Sector Agro/AP/9. MAPAS DE PRODUCTIVIDAD/",hacienda,"/",y,"/",suerte,"/",hacienda,suerte,".shp")) & ver<0.3 & !is.na(ver)){
      ...
      pts_prod <- readOGR(paste0("C:/Users/sacorreac/OneDrive - Sector Agro/AP/9. MAPAS DE PRODUCTIVIDAD/",hacienda,"/",y,"/",suerte,"/",hacienda,suerte,".shp"))#-------------------SIMPRO MAPAS DE PRODUCTIVIDAD
...
# -------------------------------------------------------------------------
# Funciones para abonadora TT (2 motores y 2 tolvas)----------------------
# -------------------------------------------------------------------------

Distrib_tolvas_tt <- function(tabla, hda, ste, area, areaR){
  tabla_recomendacion<-data.frame()
  datosArea <- function(Hda,ste) {
  ...
  data<-datosArea(hda,ste)
  #############################################################################################################
  ## 1 Producto para TT (2 tolvas)
  if(nrow(tabla)==1){
    Dosis<-tabla[1,5]
    vec<-c(0,0.22,0.25,0.255,0.265,0.3,0.35,0.375,0.4,0.43,0.45,0.47,0.49,0.5,0.55,0.56,0.57,0.6,0.625,0.65,0.7,1)
    combinaciones<-expand.grid(vec,vec)  # Solo 2 tolvas
    combinaciones$suma<-combinaciones$Var1+combinaciones$Var2
    combinaciones<-subset(combinaciones,combinaciones$suma==1)
    combinaciones$Var1<-Dosis*combinaciones$Var1
    combinaciones$Var2<-Dosis*combinaciones$Var2
    ...
    v<-c()
    for (i in 1:nrow(combinaciones)) {
      a<-which(combinaciones[i,c(1,2)]<70 & combinaciones[i,c(1,2)]>0)
      if(length(a)>0){
        v<-c(v,i)
      }else{
        next()
    ...
    Autonomia<-combinaciones
    Autonomia$Var1<-550/combinaciones$Var1  # Capacidad TT: 550kg
    Autonomia$Var2<-300/combinaciones$Var2  # Capacidad TT: 300kg
    Autonomia$suma<-apply(Autonomia[,c(1,2)],1,min)
    
    com_final<-combinaciones[which.max(Autonomia$suma),]
  }else{
    if(nrow(tabla)==2){
      ## 2 Productos para TT (2 tolvas)
      ...
      for (i in 1:nrow(combinaciones)) {
        if(length(which(combinaciones[i,]==1))==1){
          a<-which(combinaciones[i,]==1)
          b<-setdiff(c(1,2),which(combinaciones[i,]==1))  # Solo 2 tolvas
          vec<-combinaciones[i,c(1,2)]
          vec[1,a]<-vec[1,a]*dosis[1]
          vec[1,b]<-vec[1,b]*dosis[2]
          vec$fertilizantes[1]<-paste(row.names(tabla)[1],row.names(tabla)[2],sep="-")
          combinaciones1<-rbind(combinaciones1,vec)
      ...
      v<-c()
      for (i in 1:nrow(combinaciones1)) {
        a<-which(combinaciones1[i,c(1,2)]<60 & combinaciones1[i,c(1,2)]>0)
        if(length(a)>0){
          v<-c(v,i)
        }else{
          next()
      ...
      Autonomia<-combinaciones1
      Autonomia$Var1<-550/combinaciones1$Var1  # Capacidad TT: 550kg
      Autonomia$Var2<-300/combinaciones1$Var2  # Capacidad TT: 300kg
      Autonomia$suma<-apply(Autonomia[,c(1,2)],1,min)
  ...
  ############################################Generacion de tabla TT (2 tolvas)###################################################  
  if(length(which(com_final==0))==0){
    contador<-0
    for (s in 1:2) {  # Solo 2 tolvas
      tabla_recomendacion[s,1]<-Sys.Date()
      tabla_recomendacion[s,2]<-hda
      tabla_recomendacion[s,3]<-ste
      tabla_recomendacion[s,4]<-data[1,1]
    ...
    names(tabla_recomendacion)<-c("Fecha","Hacienda","Suerte","Area_Cartografia","Area real","Kg/ste-Teorico","Kg/Ha-Teorico",
                                  "Kg/ste-Real","Kg/Ha-Real","Bultos/ste-Real","Bultos/ha-Real",
                                  "TOLVA 1","TOLVA 2")  # Solo 2 tolvas
  }else{
    # Si solo hay una tolva con contenido
    tabla_recomendacion[1,1]<-Sys.Date()
    tabla_recomendacion[1,2]<-hda
    tabla_recomendacion[1,3]<-ste
    tabla_recomendacion[1,4]<-areaR
...
proj4string(shape)
shape_tt <- function(shape, recomen){
  #####Funcion para shape de TT (2 motores)
  dosis_mot <- recomen$`Kg/Ha-Real`
  
  prom <- mean(shape@data$Dosis)
  
  motores <- c("M1", "M2")  # Solo 2 motores para TT
  cont <- 1
...
for (ii in (1 : nrow(df_recomendacion))) {
  ...
  if(length(shape) == 1){
    ...
    #####Distrib tolvas - AMBOS EQUIPOS (VERION y TT)######
    # Crear estructura de carpetas para ambos tipos de abonadoras
    # VERION: {Semana}/VERION/{Semana}_mapas/{hac}{ste}/ y {Semana}/VERION/{Semana}_excel/
    # TT: {Semana}/TT/{Semana}_mapas/{hac}{ste}/ y {Semana}/TT/{Semana}_excel/
    
    dir.create(paste0(ruth_s, "/VERION/", semana_r, "_mapas/", hac, ste), recursive = TRUE, showWarnings = FALSE)
    dir.create(paste0(ruth_s, "/VERION/", semana_r, "_excel"), recursive = TRUE, showWarnings = FALSE)
    dir.create(paste0(ruth_s, "/TT/", semana_r, "_mapas/", hac, ste), recursive = TRUE, showWarnings = FALSE)
    dir.create(paste0(ruth_s, "/TT/", semana_r, "_excel"), recursive = TRUE, showWarnings = FALSE)
    
    # Rutas para shapefiles
    ruth_s_verion <- paste0(ruth_s, "/VERION/", semana_r, "_mapas/", hac, ste)
    ruth_s_tt <- paste0(ruth_s, "/TT/", semana_r, "_mapas/", hac, ste)
    
    # Rutas para archivos Excel
    ruth_e_verion <- paste0(ruth_s, "/VERION/", semana_r, "_excel")
    ruth_e_tt <- paste0(ruth_s, "/TT/", semana_r, "_excel")
    
    nu_frac <- levels(as.factor(tabla1$Fraccion))
    
    if (length(nu_frac) == 1){
      ############################### VERION (3 tolvas) #######################################
      tolvas_verion <- Distrib_tolvas(tabla1, hac, ste, area_shape, df_recomendacion$`Area a aplicar`[ii])
      if ((nrow(tolvas_verion) == 2) & (sum(tolvas_verion$`Kg/Ha-Real`) <= 150) & !is.na(tolvas_verion$'TOLVA 1'[1])){
        nombre_prod <- tolvas_verion$'TOLVA 1'[1]
        nombre_prod1 <- tolvas_verion$'TOLVA 2'[2]
        tolvas_verion$'TOLVA 1'[1] <- NA
        tolvas_verion$'TOLVA 2'[2] <- NA
        tolvas_verion$'TOLVA 2'[1] <- nombre_prod
        tolvas_verion$'TOLVA 3'[2] <- nombre_prod1
        tolvas_verion$'TOLVA 3'[1] <- NA
      }else{
        tolvas_verion <- tolvas_verion
      }
      aplicacion_verion <- shape_verion(shape, tolvas_verion)
      
      ###Descargas VERION
      writeOGR(aplicacion_verion, dsn = ruth_s_verion, layer = paste0(hac,ste,"_U"), driver = "ESRI Shapefile", overwrite_layer = TRUE)
      ruta_completa_verion <- paste0(ruth_e_verion, "/", paste0(hac,ste,"_U_tolvas.xlsx"))
      
      wb <- createWorkbook()
      addWorksheet(wb, "Hoja1")
      writeData(wb, "Hoja1", tolvas_verion)
      saveWorkbook(wb, file = ruta_completa_verion, overwrite = TRUE)
      
      ############################### TT (2 tolvas) #######################################
      tolvas_tt <- Distrib_tolvas_tt(tabla1, hac, ste, area_shape, df_recomendacion$`Area a aplicar`[ii])
      aplicacion_tt <- shape_tt(shape, tolvas_tt)
      
      ###Descargas TT
      writeOGR(aplicacion_tt, dsn = ruth_s_tt, layer = paste0(hac,ste,"_U"), driver = "ESRI Shapefile", overwrite_layer = TRUE)
      ruta_completa_tt <- paste0(ruth_e_tt, "/", paste0(hac,ste,"_U_tolvas.xlsx"))
      
      wb <- createWorkbook()
      addWorksheet(wb, "Hoja1")
      writeData(wb, "Hoja1", tolvas_tt)
      saveWorkbook(wb, file = ruta_completa_tt, overwrite = TRUE)
      
    }else{
      ##########################Fraccion 1 - VERION #########################################
      fraccion_1 <- subset(tabla1, tabla1$Fraccion == 1)
      tolvas_verion_1 <- Distrib_tolvas(fraccion_1, hac, ste, area_shape, df_recomendacion$`Area a aplicar`[ii])
      if ((nrow(tolvas_verion_1) == 2) & (sum(tolvas_verion_1$`Kg/Ha-Real`) <= 150) & !is.na(tolvas_verion_1$'TOLVA 1'[1])){
        nombre_prod <- tolvas_verion_1$'TOLVA 1'[1]
        nombre_prod1 <- tolvas_verion_1$'TOLVA 2'[2]
        tolvas_verion_1$'TOLVA 1'[1] <- NA
        tolvas_verion_1$'TOLVA 2'[2] <- NA
        tolvas_verion_1$'TOLVA 2'[1] <- nombre_prod
        tolvas_verion_1$'TOLVA 3'[2] <- nombre_prod1
        tolvas_verion_1$'TOLVA 3'[1] <- NA
      }else{
        tolvas_verion_1 <- tolvas_verion_1
      }
      aplicacion_verion_1 <- shape_verion(shape, tolvas_verion_1)
      
      ###Descargas VERION Fraccion 1
      writeOGR(aplicacion_verion_1, dsn = ruth_s_verion, layer = paste0(hac,ste,"_F1"), driver = "ESRI Shapefile", overwrite_layer = TRUE)
      ruta_completa_verion_1 <- paste0(ruth_e_verion, "/", paste0(hac,ste,"_F1_tolvas.xlsx"))
      
      wb <- createWorkbook()
      addWorksheet(wb, "Hoja1")
      writeData(wb, "Hoja1", tolvas_verion_1)
      saveWorkbook(wb, file = ruta_completa_verion_1, overwrite = TRUE)
      
      ##########################Fraccion 2 - VERION #########################################
      fraccion_2 <- subset(tabla1, tabla1$Fraccion == 2)
      tolvas_verion_2 <- Distrib_tolvas(fraccion_2, hac, ste, area_shape, df_recomendacion$`Area a aplicar`[ii])
      if ((nrow(tolvas_verion_2) == 2) & (sum(tolvas_verion_2$`Kg/Ha-Real`) <= 150) & !is.na(tolvas_verion_2$'TOLVA 1'[1])){
        nombre_prod <- tolvas_verion_2$'TOLVA 1'[1]
        nombre_prod1 <- tolvas_verion_2$'TOLVA 2'[2]
        tolvas_verion_2$'TOLVA 1'[1] <- NA
        tolvas_verion_2$'TOLVA 2'[2] <- NA
        tolvas_verion_2$'TOLVA 2'[1] <- nombre_prod
        tolvas_verion_2$'TOLVA 3'[2] <- nombre_prod1
        tolvas_verion_2$'TOLVA 3'[1] <- NA
      }else{
        tolvas_verion_2 <- tolvas_verion_2
      }
      aplicacion_verion_2 <- shape_verion(shape, tolvas_verion_2)
      
      ###Descargas VERION Fraccion 2
      writeOGR(aplicacion_verion_2, dsn = ruth_s_verion, layer = paste0(hac,ste,"_F2"), driver = "ESRI Shapefile", overwrite_layer = TRUE)
      ruta_completa_verion_2 <- paste0(ruth_e_verion, "/", paste0(hac,ste,"_F2_tolvas.xlsx"))
      
      wb <- createWorkbook()
      addWorksheet(wb, "Hoja1")
      writeData(wb, "Hoja1", tolvas_verion_2)
      saveWorkbook(wb, file = ruta_completa_verion_2, overwrite = TRUE)
      
      ##########################Fraccion 1 - TT #########################################
      tolvas_tt_1 <- Distrib_tolvas_tt(fraccion_1, hac, ste, area_shape, df_recomendacion$`Area a aplicar`[ii])
      aplicacion_tt_1 <- shape_tt(shape, tolvas_tt_1)
      
      ###Descargas TT Fraccion 1
      writeOGR(aplicacion_tt_1, dsn = ruth_s_tt, layer = paste0(hac,ste,"_F1"), driver = "ESRI Shapefile", overwrite_layer = TRUE)
      ruta_completa_tt_1 <- paste0(ruth_e_tt, "/", paste0(hac,ste,"_F1_tolvas.xlsx"))
      
      wb <- createWorkbook()
      addWorksheet(wb, "Hoja1")
      writeData(wb, "Hoja1", tolvas_tt_1)
      saveWorkbook(wb, file = ruta_completa_tt_1, overwrite = TRUE)
      
      ##########################Fraccion 2 - TT #########################################
      tolvas_tt_2 <- Distrib_tolvas_tt(fraccion_2, hac, ste, area_shape, df_recomendacion$`Area a aplicar`[ii])
      aplicacion_tt_2 <- shape_tt(shape, tolvas_tt_2)
      
      ###Descargas TT Fraccion 2
      writeOGR(aplicacion_tt_2, dsn = ruth_s_tt, layer = paste0(hac,ste,"_F2"), driver = "ESRI Shapefile", overwrite_layer = TRUE)
      ruta_completa_tt_2 <- paste0(ruth_e_tt, "/", paste0(hac,ste,"_F2_tolvas.xlsx"))
      
      wb <- createWorkbook()
      addWorksheet(wb, "Hoja1")
      writeData(wb, "Hoja1", tolvas_tt_2)
      saveWorkbook(wb, file = ruta_completa_tt_2, overwrite = TRUE)
    }
  }
...
summary(shape@data$var1.pred)

summary(shape@data$Dosis)
